...
- WooThemes Admin Interface - neuTothemes_add_admin
- Framework options panel - neuTothemes_options_page
- Framework Settings page - neuTothemes_framework_settings_page
- neuTo_load_only
- Ajax Save Action - neuTo_ajax_callback
- Generates The Options - neuTothemes_machine
- WooThemes Uploader - neuTothemes_uploader_function
- Woothemes Theme Version Checker - neuTothemes_version_checker

-----------------------------------------------------------------------------------*/

function neuTothemes_wp_head() {
   ...
   if(!isset($_REQUEST['style']))
     ...
     if ($style != '') {
      $GLOBALS['stylesheet'] = $style;
          ...
          $GLOBALS['stylesheet'] = get_option('neuTo_alt_stylesheet');
  ...
  if ($decode == 'true')
    echo '<meta name="generator" content="' . get_option('neuTo_settings_encode') . '" />';

  // Localization
  load_theme_textdomain('neuTothemes');

  // Date format
  $GLOBALS['neuTodate'] = get_option('neuTo_date');
  if ( $GLOBALS['neuTodate'] == "" )
    $GLOBALS['neuTodate'] = "d. M, Y";
...
function neuTothemes_add_admin() {

    global $query_string;
    $options =  get_option('neuTo_template');
    $themename =  get_option('neuTo_themename');
    $shortname =  get_option('neuTo_shortname');

    if ( isset($_REQUEST['page']) && $_REQUEST['page'] == 'neuTothemes' ) {

    if (isset($_REQUEST['neuTo_save']) && 'reset' == $_REQUEST['neuTo_save']) {
      global $wpdb;
      $query = "DELETE FROM $wpdb->options WHERE option_name LIKE 'neuTo_%'";
      $wpdb->query($query);
      header("Location: admin.php?page=neuTothemes&reset=true");
      die;
    }
    ...
    {
        add_object_page ('Page Title', $themename, 6,'neuTothemes', 'neuTothemes_options_page');
    }
    else
    {
        add_menu_page ('Page Title', $themename, 6,'neuTothemes_home', 'neuTothemes_options_page');
    }
    $neuTopage = add_submenu_page('neuTothemes', $themename, 'Theme Options', 6, 'neuTothemes','neuTothemes_options_page'); // Default

  // Add framework functionaily to the head individually
  add_action("admin_print_scripts-$neuTopage", 'neuTo_load_only');
  add_action("admin_print_scripts-$neuToframeworksettings", 'neuTo_load_only');
...
function neuTothemes_options_page(){

    $options =  get_option('neuTo_template');
    $themename =  get_option('neuTo_themename');
    $shortname =  get_option('neuTo_shortname');
    $manualurl =  get_option('neuTo_manual');
  ...
  $update_message = '';
    if(get_option('neuTo_theme_version_checker') == 'true') {
        $update_message = neuTothemes_version_checker ($local_version);
    }

?>
<div class="wrap" id="neuTo_container">
<div id="neuTo-popup-save" class="neuTo-save-popup"><div class="neuTo-save-save">Options Updated</div></div>
<div id="neuTo-popup-reset" class="neuTo-save-popup"><div class="neuTo-save-reset">Options Reset</div></div>
    <?php // <form method="post"  enctype="multipart/form-data"> ?>
    <form action="" enctype="multipart/form-data" id="neuToform">
        <?php
    // Rev up the Options Machine

        $return = neuTothemes_machine($options);
        ?>
      ...
      <div id="content">
        ...
        <form action="<?php echo wp_specialchars( $_SERVER['REQUEST_URI'] ) ?>" method="post" style="display:inline" id="neuToform-reset">
            ...
            <input type="hidden" name="neuTo_save" value="reset" />
            </span>
        </form>
...
function neuTothemes_framework_settings_page(){

    $options =  get_option('neuTo_settings_template');
    $themename =  get_option('neuTo_themename');
    $shortname =  get_option('neuTo_shortname');
    $manualurl =  get_option('neuTo_manual');
...
function neuTo_load_only() {

  add_action('admin_head', 'neuTo_admin_head');

  function neuTo_admin_head() {
    ...
    jQuery(document).ready(function(){
      //Color Picker
      <?php $options = get_option('neuTo_template');
    ...
    <script type="text/javascript">
      ...
      jQuery('#expand_options').click(function(){
        if(flip == 0){
          flip = 1;
          jQuery('#neuTo_container #neuTo-nav').hide();
          jQuery('#neuTo_container #content').width(755);
          jQuery('#neuTo_container .group').add('#neuTo_container .group h2').show();

          jQuery(this).text('[-]');

        } else {
          flip = 0;
          jQuery('#neuTo_container #neuTo-nav').show();
          jQuery('#neuTo_container #content').width(595);
          jQuery('#neuTo_container .group').add('#neuTo_container .group h2').hide();
          jQuery('#neuTo_container .group:first').show();
          jQuery('#neuTo_container #neuTo-nav li').removeClass('current');
          jQuery('#neuTo_container #neuTo-nav li:first').addClass('current');
        ...
        jQuery('.neuTo-radio-img-img').click(function(){
          jQuery(this).parent().parent().find('.neuTo-radio-img-img').removeClass('neuTo-radio-img-selected');
          jQuery(this).addClass('neuTo-radio-img-selected');

        });
        jQuery('.neuTo-radio-img-label').hide();
        jQuery('.neuTo-radio-img-img').show();
        jQuery('.neuTo-radio-img-radio').hide();
        jQuery('#neuTo-nav li:first').addClass('current');
        jQuery('#neuTo-nav li a').click(function(evt){

            jQuery('#neuTo-nav li').removeClass('current');
        ...
        if('<?php if(isset($_REQUEST['reset'])) { echo $_REQUEST['reset'];} else { echo 'false';} ?>' == 'true'){
          ...
          var reset_popup = jQuery('#neuTo-popup-reset');
          reset_popup.fadeIn();
      ...
      jQuery('#neuTo-popup-save').center();
      jQuery('#neuTo-popup-reset').center();
      ...
      new AjaxUpload(clickedID, {
          ...
          if(response.search('Upload Error') > -1){
            ...
            var buildReturn = '<img class="hide neuTo-option-image" id="image_'+clickedID+'" src="'+url+'" alt="" />';
            jQuery(".upload-error").remove();
      ...
      jQuery('#neuToform').submit(function(){

          function newValues() {
            var serializedValues = jQuery("#neuToform").serialize();
          ...
          var data = {
            <?php if(isset($_REQUEST['page']) && $_REQUEST['page'] == 'neuTothemes_framework_settings'){ ?>
            type: 'framework',
            <?php } ?>
            action: 'neuTo_ajax_post_action',
          ...
          jQuery.post(ajax_url, data, function(response) {
            var success = jQuery('#neuTo-popup-save');
...
add_action('wp_ajax_neuTo_ajax_post_action', 'neuTo_ajax_callback');

function neuTo_ajax_callback() {
  ...
  if(isset($_POST['type'])){
    ...
    elseif($_POST['type'] == 'framework'){
      ...
      foreach($output as $id => $value){
        ...
        if(!isset($output['neuTo_show_options'])){
          update_option('neuTo_show_options','false');
        }
        elseif ( $id == 'neuTo_show_options' AND $value == 'true') { update_option($id,'true'); }

        // Woo Theme Version Checker Save
        if(!isset($output['neuTo_theme_version_checker'])){
          update_option('neuTo_theme_version_checker','false');
        }
        elseif ( $id == 'neuTo_theme_version_checker' AND $value == 'true') { update_option($id,'true'); }


        // Woo Core update Save
        if(!isset($output['neuTo_framework_update'])){
          update_option('neuTo_framework_update','false');
        }
        elseif ( $id == 'neuTo_framework_update' AND $value == 'true') { update_option($id,'true'); }

        // Woo Buy Themes Save
        if(!isset($output['neuTo_buy_themes'])){
          update_option('neuTo_buy_themes','false');
        }
        elseif ( $id == 'neuTo_buy_themes' AND $value == 'true') { update_option($id,'true'); }
  ...
  $query = "SELECT * FROM $wpdb->options WHERE option_name LIKE 'neuTo_%' AND
        option_name != 'neuTo_options' AND
        option_name != 'neuTo_template' AND
        option_name != 'neuTo_custom_template' AND
        option_name != 'neuTo_settings_encode' AND
        option_name != 'neuTo_export_options' AND
        option_name != 'neuTo_import_options' AND
        option_name != 'neuTo_framework_version' AND
        option_name != 'neuTo_manual' AND
        option_name != 'neuTo_shortname'";
  ...
  foreach ($results as $result){
      ...
      if(is_serialized($value)) {
        ...
        $value = $temp_options;
        $neuTo_array[$name] = $neuTo_array_option;
      } else {
        $neuTo_array[$name] = $value;
      }
  ...
  update_option('neuTo_options',$neuTo_array);
  update_option('neuTo_settings_encode',$output);
...
function neuTothemes_machine($options) {
  ...
  foreach ($options as $value) {
    ...
    case 'text':
      ...
      if ( $std != "") { $val = $std; }
      $output .= '<input class="neuTo-input" name="'. $value['id'] .'" id="'. $value['id'] .'" type="'. $value['type'] .'" value="'. $val .'" />';
    break;

    case 'select':

      $output .= '<select class="neuTo-input" name="'. $value['id'] .'" id="'. $value['id'] .'">';
    ...
    case 'textarea':
      ...
      $output .= '<textarea class="neuTo-input" name="'. $value['id'] .'" id="'. $value['id'] .'" cols="'. $cols .'" rows="8">'.$ta_value.'</textarea>';
    ...
    case "radio":
       ...
       foreach ($value['options'] as $key => $option)
       {
        ...
        $output .= '<input class="neuTo-input neuTo-radio" type="radio" name="'. $value['id'] .'" value="'. $key .'" '. $checked .' />' . $option .'<br />';
    ...
    case "checkbox":
      ...
      $output .= '<input type="checkbox" class="checkbox neuTo-input" name="'.  $value['id'] .'" id="'. $value['id'] .'" value="true" '. $checked .' />';
    ...
    case "multicheck":
      ...
      $neuTo_key = $value['id'] . '_' . $key;
      $saved_std = get_option($neuTo_key);
      ...
      elseif( $std == $key) {
        ...
        $checked = '';                                                                                    }
      $output .= '<input type="checkbox" class="checkbox neuTo-input" name="'. $neuTo_key .'" id="'. $neuTo_key .'" value="true" '. $checked .' /><label for="'. $neuTo_key .'">'. $option .'</label><br />';

      }
    break;
    case "upload":

      $output .= neuTothemes_uploader_function($value['id'],$value['std'],null);

    break;
    case "upload_min":

      $output .= neuTothemes_uploader_function($value['id'],$value['std'],'min');
    ...
    case "color":
      ...
      $output .= '<input class="neuTo-color" name="'. $value['id'] .'" id="'. $value['id'] .'" type="text" value="'. $val .'" />';
    break;
    ...
    case "typography":
      ...
      $output .= '<select class="neuTo-typography neuTo-typography-size" name="'. $value['id'].'_size" id="'. $value['id'].'_size">';
        for ($i = 9; $i < 71; $i++){
      ...
      $output .= '<select class="neuTo-typography neuTo-typography-unit" name="'. $value['id'].'_unit" id="'. $value['id'].'_unit">';
      ...
      */
      ...
      $output .= '<select class="neuTo-typography neuTo-typography-face" name="'. $value['id'].'_face" id="'. $value['id'].'_face">';
      ...
      $output .= '<select class="neuTo-typography neuTo-typography-style" name="'. $value['id'].'_style" id="'. $value['id'].'_style">';
      ...
      $output .= '<input class="neuTo-color neuTo-typography neuTo-typography-color" name="'. $value['id'] .'_color" id="'. $value['id'] .'_color" type="text" value="'. $val .'" />';
    ...
    case "border":
      ...
      $output .= '<select class="neuTo-border neuTo-border-width" name="'. $value['id'].'_width" id="'. $value['id'].'_width">';
        for ($i = 0; $i < 21; $i++){
          if($val == $i){ $active = 'selected="selected"'; } else { $active = ''; }
          $output .= '<option value="'. $i .'" ' . $active . '>'. $i .'px</option>'; }
      $output .= '</select>';
      ...
      $output .= '<select class="neuTo-border neuTo-border-style" name="'. $value['id'].'_style" id="'. $value['id'].'_style">';
      ...
      $output .= '<div id="' . $value['id'] . '_color_picker" class="colorSelector"><div></div></div>';
      $output .= '<input class="neuTo-color neuTo-border neuTo-border-color" name="'. $value['id'] .'_color" id="'. $value['id'] .'_color" type="text" value="'. $val .'" />';
    ...
    case "images":
      ...
      foreach ($value['options'] as $key => $option)
       ...
       $i++;
         ...
         $selected = '';
           if($select_value != '') {
            if ( $select_value == $key) { $checked = ' checked'; $selected = 'neuTo-radio-img-selected'; }
            } else {
            if ($value['std'] == $key) { $checked = ' checked'; $selected = 'neuTo-radio-img-selected'; }
            elseif ($i == 1  && !isset($select_value)) { $checked = ' checked'; $selected = 'neuTo-radio-img-selected'; }
            elseif ($i == 1  && $value['std'] == '') { $checked = ' checked'; $selected = 'neuTo-radio-img-selected'; }
            else { $checked = ''; }
          }

        $output .= '<span>';
        $output .= '<input type="radio" id="neuTo-radio-img-' . $value['id'] . $i . '" class="checkbox neuTo-radio-img-radio" value="'.$key.'" name="'. $value['id'].'" '.$checked.' />';
        $output .= '<div class="neuTo-radio-img-label">'. $key .'</div>';
        $output .= '<img src="'.$option.'" alt="" class="neuTo-radio-img-img '. $selected .'" onClick="document.getElementById(\'neuTo-radio-img-'. $value['id'] . $i.'\').checked = true;" />';
        $output .= '</span>';
    ...
    case "heading":
      ...
      $jquery_click_hook = "neuTo-option-" . $jquery_click_hook;
//      $jquery_click_hook = "neuTo-option-" . str_replace("&","",str_replace("/","",str_replace(".","",str_replace(")","",str_replace("(","",str_replace(" ","",strtolower($value['name'])))))));
    ...
    if ( is_array($value['type'])) {
      foreach($value['type'] as $array){
          ...
          if($array['type'] == 'text') { // Only text at this point
             ...
             $output .= '<input class="input-text-small neuTo-input" name="'. $id .'" id="'. $id .'" type="text" value="'. $std .'" />';
...
function neuTothemes_uploader_function($id,$std,$mod){
  ...
  if($mod != 'min') {
      $val = $std;
            ...
            $uploader .= '<input class="neuTo-input" name="'. $id .'" id="'. $id .'_upload" type="text" value="'. $val .'" />';
  }
  ...
  if(!empty($upload)){
    ...
    //$upload = cleanSource($upload); // Removed since V.2.3.7 it's not showing up
      $uploader .= '<a class="neuTo-uploaded-image" href="'. $thumb_url . '">';
...
function neuTothemes_version_checker ($local_version) {

  // Get a SimplePie feed object from the specified feed source.
  $rss = fetch_feed('http://www.neuTothemes.com/?feed=updates&theme=' . get_option('template'));
  ...
  if(isset($pieces_rss[0]) && $pieces_rss[0] != 0) {
    ...
    if ($version_sentinel == true) {
      $update_message = '<div class="update_available">Theme update is available (v.' . $latest_version_via_rss . ') - <a href="http://www.neuTothemes.com/amember">Get the new version</a>.</div>';
    }
    else {
      $update_message = '';
    }
